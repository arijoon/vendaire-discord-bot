...
@injectable()
export class RockPaperSiccors implements ICommand, IHasHelp {

  _command: string = commands.games.rps;
  _subscription: IDisposable[] = [];

  _actions: any = { 'rock': 'r', 'paper': 'p', 'sissor': 's' };
  _reverse: any = { 'r': 'rock', 'p': 'paper', 's': 'scissor' };
  ...
  attach(): void {
    this._subscription.push(this._client
      ...
      .subscribe(imsg => {
        ...
        if (!action) {
          imsg.send(`Can't play this game with ${content}`);
  ...
  getHelp(): IHelp[] {
    return [
      {
        Key: this._command,
        Message: 'Play Rock/Paper/Sissor',
        Usage: `${this._command} ACTION (available actions: ${Object.keys(this._actions).join('|')})`
      }
    ]
  }
